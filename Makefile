# This Makefile is for the WWW::WikiSpace extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/WWW/WikiSpace.pm]
#     AUTHOR => q[Leonty Chudinov <leonty@inbox.ru>]
#     NAME => q[WWW::WikiSpace]
#     PREREQ_PM => { WWW::Mechanize=>q[1.0] }
#     VERSION_FROM => q[lib/WWW/WikiSpace.pm]
...
FULLEXT = WWW/WikiSpace
BASEEXT = WikiSpace
PARENT_NAME = WWW
DLBASE = $(BASEEXT)
VERSION_FROM = lib/WWW/WikiSpace.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
...
TO_INST_PM = lib/WWW/WikiSpace.pm

PM_TO_BLIB = lib/WWW/WikiSpace.pm \
	blib/lib/WWW/WikiSpace.pm
...
DISTNAME = WWW-WikiSpace
DISTVNAME = WWW-WikiSpace-0.01
...
manifypods : pure_all  \
	lib/WWW/WikiSpace.pm \
	lib/WWW/WikiSpace.pm
	$(NOECHO) $(POD2MAN) --section=3pm --perm_rw=$(PERM_RW)\
	  lib/WWW/WikiSpace.pm $(INST_MAN3DIR)/WWW::WikiSpace.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  blib/lib/WWW/WikiSpace.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         WWW-WikiSpace' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/WWW/WikiSpace.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    WWW::Mechanize:                1.0' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension for posting to WikiSpace</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Leonty Chudinov &lt;leonty@inbox.ru&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="WWW-Mechanize" VERSION="1,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/WWW/WikiSpace.pm blib/lib/WWW/WikiSpace.pm 
	$(NOECHO) $(TOUCH) $@
